var app = angular.module("my-app",[]);
    app.factory('interceptor',[function(){
		...
		app.controller('AdminCtrl',function($scope,$http){
			$http.get("https://localhost:44358/api/admins/"+localStorage.getItem("user")).then(function(resp){
			debugger;
		...
		app.controller('AdminEditCtrl',function($scope,$http){
			$http.Post("https://localhost:44358/api/admins/{username}/update"+localStorage.PostItem("user"))+localStorage.postItem("admins").then(function(resp){
			debugger;
				$scope.admin = resp.data;
			},
			function(err){
